...

{
    ...
    public function room_booked_checkout(Request $request){
        ...
        $mailData_one = [
            ...
            "type" => $rooms->title,
            "guest" => ((int)$request->adults + (int)$request->childrens),
        ];
        $template_one = 'email-template.booking-email';
        $subject_one = "Booking Confirm!";
        $checkStatus_one = $this->sendMail($request->email,$mailData_one,$template_one,$subject_one);
    ...
    public function get_total_room_available(Request $request){
        ...
        $rommNos = RoomNo::select('room_no')->where('room_id', $request->id)->where('room_status','active')->pluck('room_no')->toArray();
        ...
        if($allReq['start_date'] !='' && $allReq['end_date'] !=''){
            ...
            if(($allReq['end_date'] >= $allReq['start_date']) && ($allReq['start_date'] <= $allReq['end_date'])){
                ...
                if($order->get()->count() > 0){
                    ...
                    // dd((count($rommNos) - count($roomNoGet)));
...
// }
    ...
    public function searchByfilter(Request $request){
        ...
        if ($request->facilities) {
            ...
            $roomsData = Order::where(function ($query) use ($fromDate, $toDate) {
             ...
             ->select('room_id', DB::raw('count(room_no) as room_count'))
            ...
            $rooms = $roomsData->pluck('room_id')->toArray();

            $roomCountMap = $roomsData->pluck('room_count', 'room_id')->toArray();



            // Step 2: Count room_no entries for each room_id in the room_no table only for $rooms ids

            $roomCountsInRoomNo = DB::table('room_no')

                ->select('room_id', DB::raw('count(*) as room_no_count'))
            ...
            // Step 3: Filter room_id values based on the condition that their count is greater than or equal to $roomCount

            $filteredRoomIds = $roomCountsInRoomNo->filter(function ($room) use ($roomCountMap) {

                return isset($roomCountMap[$room->room_id]) && $room->room_no_count <= $roomCountMap[$room->room_id];

            })->pluck('room_id')->toArray();

            dd($filteredRoomIds);

            // Step 4: Combine the filtered room_ids with the existing query logic

            $roomsQuery = Room::whereIn('id', $filteredRoomIds)
            ...
            $bookedRooms = Order::where(function ($query) use ($fromDate, $toDate) {
            ...
            ->join('room_no', 'orders.room_id', '=', 'room_no.room_id')

            ->groupBy('orders.room_id')

            ->select('orders.room_id', DB::raw('count(room_no.id) as booked_room_count'))
            ...
            // Validate and cast request inputs

            $noAdult = (int) $request->no_adult;

            $noChildren = $request->has('no_children') ? (int) $request->no_children : null;



            // Build the room query

            $roomsQuery = Room::where(function($query) use ($bookedRooms) {

                foreach ($bookedRooms as $roomId => $data) {

                    $query->orWhere(function($q) use ($roomId, $data) {

                        $q->where('id', $roomId)

                          ->whereRaw('room_quantity > ?', [$data['booked_room_count']]);
            ...
            ->orWhereNotIn('id', array_keys($bookedRooms))

            ->where('adults', '>=', $noAdult);



            if (!is_null($noChildren)) {

                $roomsQuery->where('childrens', '>=', $noChildren);
