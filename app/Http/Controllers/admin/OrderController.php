...

{
    ...
    public function assign_room_no_backup(Request $request){
    ...
    public function assign_room_no(Request $request){
        ...
        $rommNos = RoomNo::select('room_no')->where('room_id', $request->data)->where('room_status','active')->pluck('room_no')->toArray();
        ...
        $getData = $this->getAvailableRoomNos($allReq,$rommNos);
    ...
    public function getAvailableRoomNos($allReq,$rommNos){
        ...
        if($allReq['start_date'] !='' && $allReq['end_date'] !=''){
            ...
            if(($allReq['end_date'] >= $allReq['start_date']) && ($allReq['start_date'] <= $allReq['end_date'])){
                ...
                if($order->get()->count() > 0){
                    ...
                    foreach ($rommNos as $rommNoskey => $rommNosvalue) {

                        if(!in_array($rommNosvalue,$roomNoGet)){

                            $availroomNo[] = $rommNosvalue;
    ...
    public function get_room_count(Request $request){
        ...
        foreach ($rooms as $key => $value) {
            ...
            $html .= '<option value="'.$value->id.'">'.$value->title.' (Availble '.$this->getAvailableRoomForAdmin($request->all(),$value->id).')</option>';
    ...
    public function getAvailableRoomForAdmin($allReq,$roomId){

        $rommNos = RoomNo::select('room_no')->where('room_id', $roomId)->where('room_status','active')->pluck('room_no')->toArray();

        if($allReq['start_date'] !='' && $allReq['end_date'] !=''){

            if(($allReq['end_date'] >= $allReq['start_date']) && ($allReq['start_date'] <= $allReq['end_date'])){

                $order = Order::where(function ($query) use ($allReq,$roomId) {
    ...
    public function room_change(request $request){
        ...
        $avilRoom = $this->getAvailableRoomForAdmin($request->all(),$request->id);
    ...
    public function store(Request $request){
        ...
        $order->groos_amount = $request->amount;

        $order->discount     = '';

        $order->nett_amount  = $request->amount;
